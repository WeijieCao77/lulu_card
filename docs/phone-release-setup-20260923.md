...
- 用户报告已在 RAM 创建 `lulucard-sms` 并下载 AccessKey CSV，另存了本地副本；本记录不包含也不读取密钥。已在 RAM 创建自定义策略 `LuluCardSmsVerifyOnly`，控制台源代码核对仅含 `dypns:SendSmsVerifyCode` 和 `dypns:CheckSmsVerifyCode`。RAM 用户列表持续加载，子用户是否存在及是否绑定策略仍待核验；必须授予最小权限后才能调用接口。当前没有调用发送接口，也没有产生测试短信费用。
