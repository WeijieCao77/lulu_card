...
- 用户已在 RAM 创建 `lulucard-sms` 并下载 AccessKey CSV，另存了本地副本；本记录不包含密钥。已在 RAM 创建自定义策略 `LuluCardSmsVerifyOnly`，控制台源代码核对仅含 `dypns:SendSmsVerifyCode` 和 `dypns:CheckSmsVerifyCode`。用户确认已将该策略绑定到 `lulucard-sms`；随后使用此专用账号的 AccessKey 实际发送及核验短信通过，也证明了该权限绑定可用。
...
- 短信费用防刷（2026-09-23 补）：绑定发码必须携带已存在的账号 ID，不带账号或账号不存在一律不发；每个账号 24 小时内最多触发 5 条绑定码；全站 24 小时内的发码总量受 `SMS_DAILY_CAP` 限制（未设置为 2000 条，按 0.06 元/条约 120 元/天；设为 `0` 可紧急停发；非数字按默认值）。达到上限时绑定和找回都暂停并提示玩家，服务端日志记 `sms: daily cap ... reached`，`/api/admin/sms` 的 stats 显示 `cap` 与 `sent24`。全站计数用独立的 PostgreSQL 事务锁串行化，只锁住预留这一步。开瓦包 `bf8f6df` 没有这几项限制，属于噜噜卡新增；上线前按预期新玩家数量确认 `SMS_DAILY_CAP`。
...
- DeepSeek 独立审查了正式版手机号流程；复核其报告后，修正了有实际影响的失败路径：阿里云校验接口错误与错误验证码分开处理，服务商故障不消耗本地试错次数；手机号找回在返回 ID 前重新确认绑定记录、解密结果和账号存在，记录损坏时不会吞掉验证码。
- 正式版建档页对账号找回的说明已改为与手机号功能一致。内测版仍保留当前说明。
- 验收已通过：账号绑定、正式门槛、手机号找回、错码与重放拒绝、并发试错、短信服务故障、损坏记录回滚、TypeScript 检查与完整构建。真实短信发送和核验使用专用 RAM AccessKey 完成；该测试没有使用线上玩家数据库。
- 仍需在隔离的预发布环境连接真实 PostgreSQL，从手机浏览器和电脑浏览器走一遍完整建档、收码、绑定与换设备找回，再决定切换正式策略。当前 `RELEASE_STAGE` 保持 `demo`，未推送或部署。
