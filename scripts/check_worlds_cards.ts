...
assert.equal(BASE_PLAYER_CARDS.filter(c=>c.rarity==='gold').length,81)
assert.equal(BASE_PLAYER_CARDS.filter(c=>c.rarity==='silver').length,196)
assert.equal(BASE_PLAYER_CARDS.filter(c=>c.rarity==='bronze').length,400)
