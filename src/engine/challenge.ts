...
export function answerPool(kind: ChallengeKind): string[] {
  if (kind === 'player') {
  ...
  if (kind === 'team') return WORLD_TEAMS.filter((t) => TIER1.has(t.id) && !!DOSSIER.logos?.[t.id]).map((t) => t.id)
...
export function imgOf(kind: ChallengeKind, id: string): string | undefined {
  if (kind === 'player') {
    ...
    return d?.img ? `lol/faces/${d.img}` : undefined
  }
  return kind === 'team' ? `lol/logos/${id}.webp`
    : kind === 'map' ? 'lol/minimaps/rift.png'
      : `lol/champions/${id}.png`
}
