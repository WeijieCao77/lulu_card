...
export function doTransfer(
    ...
    if (from.id !== state.myTeam && from.roster.length < 5) {
      ...
      if (cover) {
        ...
        state.news.push({
          ...
          text: `${from.name} 紧急签下辅助 ${cover.ign}（${cover.overall}）填补空缺。`,
        })
      }
    }
    from.budget += fee
  ...
  state.news.push({
    ...
    text: (fee > 0
      ...
      : `${to.name} 免转会费签下辅助 ${p.ign}（${p.overall}）。`)
      + (watched ? ' 你之前在接触他。' : ''),
...
export function releasePlayer(state: GameState, p: Player): string {
  ...
  state.news.push({
    day: state.day, kind: 'transfer',
    text: `${from?.name ?? '某队'} 与 ${p.ign} 解除合同，该选手成为辅助。`,
    important: from?.id === state.myTeam,
  })
  return `${p.ign} 已离队，成为辅助。`
}
...
function weakestRole(state: GameState, team: Team): { role: Player['role']; strength: number } | null {
  ...
  // 辅助 is not a position to fill — in the data it means "vlr never recorded
  ...
  const roles: Player['role'][] = ['上单', '打野', '中单', '下路']
...
export function enquireAbout(state: GameState, playerId: string): string {
  ...
  const p = state.players[playerId]
  if (!p) return '找不到这名选手。'
  if (!p.teamId) return '他是辅助，直接报价即可。'
