...
export type Region = 'LCS' | 'LEC' | 'LCK' | 'LPL' | 'LCP' | 'CBLOL'
...
export const REGION_CN: Record<Region, string> = {
  LPL: 'LPL · 中国', LCK: 'LCK · 韩国', LEC: 'LEC · 欧洲', LCS: 'LCS · 北美', LCP: 'LCP · 亚太', CBLOL: 'CBLOL · 巴西',
}
...
export const ATTR_CN: Record<keyof Attrs, string> = {
  aim: '操作', reaction: '对线', awareness: '意识', utility: '发育',
  clutch: '关键发挥', teamwork: '团战', communication: '协同', igl: '运营',
}
...
export interface Player {
   ...
   *
   * 位置是错误的粒度。捷风打得好的上单换到夜露就不是同一个人，而游戏此前
...
export interface EdgeBreakdown {
  ...
  tacticsDef: number
  /**
   * the shape of the five — 双决斗 / 双下路 / 双中单 — before any dial;
...
export interface Fixture {
  ...
  day: number
  stage: StageKey
  /** league key ('VCT LCS'), or an international event name */
  comp: string
  teamA: string
  teamB: string
  bo: 1 | 3 | 5
...
/** 这局是不是噜噜卡的对战。版本 / 打法三角 / 英雄熟练度都对它让路。 */
