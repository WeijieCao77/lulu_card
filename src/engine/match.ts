...
// 上单 was 1.15 while every real duelist in a fifth slot sat on a default
...
const CORE_ROLES: Role[] = ['上单', '打野', '中单', '下路']
const GAP_COST: Record<string, number> = { 中单: 7, 下路: 5, 打野: 4, 上单: 4 }

/**
 * How well a five covers the map between them.
 *
 * The requirement is four roles — duelist, initiator, controller, sentinel —
 * and nothing else. 辅助 is not a fifth one: in the data it means vlr never
...
export function buildLineup(
  ...
  // 英雄熟练度只做在经理模式里：噜噜卡的卡按槽位排，不该因为「这张卡没练过
  // 这个英雄」而变弱
...
export function runVeto(
  ...
  // LoL plays every game on the Rift; the series still uses the original BO rules.
  if (pool.length === 1) return { maps: Array(bo).fill(pool[0]), log: [] }
